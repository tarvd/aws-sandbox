...
def main() -> None:
    ...
    cols_df["is_numeric"] = [
        True if x in ["INTEGER", "FLOAT"] else False for x in cols_df["column_type"]
    ]
    ...
    col_grouping = cols_df.loc[
        cols_df["formatted_name"] == col_name_grouping, "column_name"
    ].item()
    sql_function = {
        "Count": "COUNT",
        "Sum": "SUM",
    ...
    if col_name_argument == "*":
        ...
        col_argument = cols_df.loc[
            cols_df["formatted_name"] == col_name_argument, "column_name"
        ].item()
