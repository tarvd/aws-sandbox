...
def main():
    ...
    try: 
        ...
        logger.info(f"Beginning job: {job_name}")
        ...
        for filename in source_file_list:
            ...
            logger.info(f"Number of rows loaded to Spark DF: {source_num_rows}")
            ...
            logger.info(f"Reading target table {target_db}.{target_table} row IDs")
            ...
            logger.info("Dropping duplicates and already-existing rows")
            df = df.withColumn(
                "row_hash",
                sha2(
                    concat_ws(
            ...
            logger.info(f"Number of rows that will be loaded to target: {filtered_num_rows}")
            ...
            logger.info(f"Adding row to Processed Data Log for {filename}")
