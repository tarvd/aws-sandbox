...
#     # Check HWM, see if event_id is higher and set new HWM if so, update HWM after insert, every time.
#     current_event_hwm = max(event_id, event_id_hwm)
#     insert_sql = f"""
...
#             {current_event_hwm} as current_event_hwm,
...
def get_process_event_id_hwm(event_consumer: str, athena_client, log_table: str = "metadata.data_process_log") -> int:
    read_sql = f"""
        ...
        limit 1
    """
    scalar = run_athena_query(read_sql, athena_client)["Rows"][0][0]
    event_hwm = scalar.get("VarCharValue", -1)
    return event_hwm
