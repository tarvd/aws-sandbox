...
resource "aws_athena_workgroup" "dbt" {
  name = var.athena_workgroup_dbt.name

  configuration {
    enforce_workgroup_configuration    = var.athena_workgroup_dbt.enforce_workgroup_configuration
    publish_cloudwatch_metrics_enabled = var.athena_workgroup_dbt.publish_cloudwatch_metrics_enabled

    result_configuration {
      expected_bucket_owner = var.aws_account_id
      output_location       = "s3://${aws_s3_bucket.athena_results.bucket}/${var.athena_workgroup_dbt.name}/"

      encryption_configuration {
        encryption_option = var.athena_workgroup_dbt.encryption_option
      }

      acl_configuration {
        s3_acl_option = var.athena_workgroup_dbt.acl_option
      }
    }
  }
}
