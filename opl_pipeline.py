...
class HttpRequest:
    ...
    def make_request(self) -> None:
        ...
        self.download_start_at = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S %Z")
        try:
            self.response = requests.get(self.url)
            self.download_end_at = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S %Z")
...
class S3Upload:
    ...
    def upload_file(self) -> None:
        ...
        self.upload_start_at = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S %Z")
        try:
            ...
            self.upload_end_at = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S %Z")
