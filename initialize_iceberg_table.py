...
def create_iceberg_from_df(df: pd.DataFrame, database: str, table: str, table_location: str, temp_path: str) -> None:
...
def initialize_iceberg_table(s3_path: str, database: str, table: str, table_location: str, temp_path: str) -> None:
    if wr.catalog.does_table_exist(database, table):
        logging.info(f"Table already exists in Glue catalog.")
        logging.info(f"Program ended after finding a Glue catalog table {database}.{table}")
    else:
...
def main() -> None:
    ...
    table = "lifter"
    table_location = "s3://tdouglas-data-prod-useast2/data/raw/openpowerlifting/lifter/iceberg/"
